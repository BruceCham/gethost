...
const getUrlIp = async () => {
    let res = await axios.get('https://websites.ipaddress.com/github.com')
    let val = res.data.match(/(?<=\.com\/ipv4\/)[\d\.]+/g)
    return [val[0], 'github.com']
}
// 2. 获取github域名查询
const getDomainIp = async () => {
    let res = await axios.get('https://websites.ipaddress.com/github.global.ssl.fastly.net')
    let val = res.data.match(/(?<=\.com\/ipv4\/)[\d\.]+/g)
    return [val[0], 'github.global.ssl.fastly.net']
}
// 3. 获取github静态资源ip
const getStaticIp = async () => {
    let res = await axios.get('https://websites.ipaddress.com/assets-cdn.github.com')
    let val = res.data.match(/(?<=\.com\/ipv4\/)[\d\.]+/g)
